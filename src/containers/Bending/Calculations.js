...
const getBendOffset=(
    ...
    return radius*Math.tan(
        Number(angle)*Math.PI/360
    );
};
...
export const calculateOuterLengthToEnd=(
    profile,
    bendIndex,
    bendSide
)=>{
    if(bendIndex<0) return 0;
...
export const calculateBendingMachineParams=({
    ...
    const angle1=Math.asin(
        rTool/lPivotToCenter
    );

    const angle2=
        Math.PI/2-rad+angle1;

    const deltaShelfInOut=
        t*Math.tan(rad/2);
